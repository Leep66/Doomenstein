...
{
	float ambient = 0.35;
	float directional = 0.85 * saturate(dot(normalize(input.worldNormal.xyz), -SunDirection));
	float4 lightResult = (ambient + directional) * SunColor;

	float4 lightColor = float4(lightResult.xyz, 1.0f);
